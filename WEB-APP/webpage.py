...
#pdek#devices_added = Gauge('mycas_new_activation', 'New activation', registry=registry)
#pdek#pushgateway_url = 'http://192.168.56.11:9091'
...
def device_keys():
    ...
    try:
        ...
        logger.info(f"Message sent to Kafka topic {topic}: {message}")
        new_devices += 1
        #pdek#devices_added.set(new_devices)
        #pdek#push_to_gateway(pushgateway_url, job='device_keys', registry=registry)
        #pdek#print(new_devices)
    except KafkaError as e:
        logger.error(f"Failed to send message to Kafka: {e}")
    # Acquire a connection from the pool
    connection = connection_pool.get_connection()
